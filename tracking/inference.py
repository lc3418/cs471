...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        allPossible = util.Counter();
        for p in self.legalPositions:
            prev_p, distribution = self.beliefs[p], self.getPositionDistribution(self.setGhostPosition(gameState, p));

            for key, value in distribution.items():
                allPossible[key] = allPossible[key] + value * prev_p;
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        self.particles = [];
        num_of_particles, pos = 0, self.legalPositions;
    ...
    def observe(self, observation, gameState):
        ...
        #util.raiseNotDefined()
        # self.particles = [];
        next_dist, b_dist = util.Counter(), self.getBeliefDistribution();

        if noisyDistance is not None:
            for p in self.legalPositions:
                dist = util.manhattanDistance(pacmanPosition, p);
                if emissionModel[dist] > 0:
                    next_dist[p] = emissionModel[dist] * b_dist[p];
            if next_dist.totalCount() != 0:
                next_dist.normalize();
                self.particles = [];
                for i in range(self.numParticles):
                    self.particles.append(util.sample(next_dist));
            else:
                self.initializeUniformly(gameState);
        else:
            self.particles = [];
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.particles:
            ...
            new_particles.append(util.sample(new_dist));
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        temp = itertools.product(self.legalPositions, repeat = self.numGhosts);
    ...
    def observeState(self, gameState):
        ...
        for p in self.particles:
            ...
            for ghost in ghost_eaten:
                p = self.getParticleWithGhostInJail(p, ghost);
            for ghost in range(self.numGhosts):
                if ghost not in ghost_eaten:
                    dist = util.manhattanDistance(p[ghost], pacmanPosition);
                    temp *= emissionModels[ghost][dist];
            new_dist[p] += temp;
            
        if new_dist.totalCount() != 0:
            new_dist.normalize();
            l = len(self.particles);
            for p in range(l):
                self.particles[p] = util.sample(new_dist);
        else:
            self.initializeParticles();
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for ghost in range(self.numGhosts):
                ...
                pos_dist = getPositionDistributionForGhost(ghost_pos, ghost, self.ghostAgents[ghost]);
                newParticle[ghost] = util.sample(pos_dist);
